...
for i in range(1, 6):
    url = 'https://wth2052.tistory.com/?page='+str((i))

    data = requests.get(url, headers=headers)
    soup = BeautifulSoup(data.text, 'html.parser')


    blogs = soup.select('#main > div.area_category.category_type_notice.category_search_list > ul > li')
    ...
    for blog in blogs:
        ...
        if a is not None:
            ...
            summary = blog.select_one('a > div > p').text
